# ==========================================
# PARTE 1: INFRAESTRUCTURA DE RED, CONFIGURACIÓN CORE Y CONEXIONES
# ==========================================
import os
import re
import random
...
# Servidor Falso (Opcional, si lo usas para algo específico)
def run_fake_server():
...
def init_db():
    ...
    ''')
    try:
        cursor.execute('''ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS ultimo_uso 
        DOUBLE PRECISION DEFAULT 0''')
    except: 
        pass
    try:
        cursor.execute('''ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS es_staff 
        INTEGER DEFAULT 0''')
    except: 
        pass
    cursor.execute('''
...
def guardar_grupo_staff_db(chat_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('INSERT INTO config_servidor (clave, valor_id) VALUES (%s, %s) ON CONFLICT (clave) DO UPDATE SET valor_id = %s',
                   ('grupo_staff', chat_id, chat_id))
    conn.commit()
    cursor.close()
    conn.close()
...
def recuperar_grupo_staff_db():
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT valor_id FROM config_servidor WHERE clave = %s', ('grupo_staff',))
        result = cursor.fetchone()
        cursor.close()
        conn.close()
...
def update_user_credits(user_id, nuevos_creditos):
    ...
    conn.close()
# ==========================================
# PARTE 2: GATEWAY DE SEGURIDAD (ANTIFLOOD), VALIDACIONES
# ==========================================
def check_user_access(message, cost=1):
    ...
    if not datos_usuario:
        bot.reply_to(message, "Acceso Denegado. Registrate con /register tu_nombre.")
        return False
...
# ==========================================
# PARTE 3: ADMINISTRACIÓN Y STAFF
# ==========================================
...
# ==========================================
# PARTE 4: COMANDOS PÚBLICOS Y LOGIC
# ==========================================
...
def callback_listener_buttons(call):
    ...
    # Hack para que el handler de comandos sepa que fue un callback
...
def contact_support_team(message):
    ...
    if not datos_usuario: 
        return
    if len(args) < 2:
        bot.reply_to(message, "\n**Uso correcto:** <code>/soporte Tu mensaje aquí</code>", parse_mode="HTML")
        return
...
def generate_cards(message):
    ...
    try:
        ...
        if len(bin_number) < 6:
            bot.reply_to(message, "El BIN debe tener al menos 6 digitos.")
            return
        bin_base = bin_number
        generated_list = []
...
# ==========================================
# INICIO DEL BOT (AQUÍ VA AL FINAL)
# ==========================================
while True:
    try:
        bot.delete_webhook()
