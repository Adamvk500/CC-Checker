...
# 🔑 PEGA AQUÍ TU API KEY DE BINLIST.IO
BINLIST_API_KEY = "TU_API_KEY_DE_BINLIST_AQUI" 

# Simulación de rotación de User-Agent y Proxies
ua = UserAgent()
PROXY_LIST = [
 "http://proxy1:port",
 "http://proxy2:port",
...
# ==========================================
# # NUEVA FUNCIÓN: CONSULTA PROFESIONAL A BINLIST.IO
# ==========================================
def get_bin_info_binlist(bin_number):
    """
    Consulta Binlist.io para obtener datos reales del banco.
    Retorna un diccionario con los datos o None si falla.
    """
    url = f"https://api.binlist.net/{bin_number}/"
    headers = {
        "Authorization": f"Bearer {BINLIST_API_KEY}",
...
def reject_bizum_ticket(message):
 ...
 bot.send_message(chat_origen_salva, f"🔑 <b>¡BIZUM RECHAZADO!</b>\n─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─\n👤 Cliente: <code>{alias}</code>\n📊 Estado: <b>No Recibido / Falso</b>\n🚫 🔼🗻 Resolución: <i>Ticket cerrado. No se ha encontrado ningun ingreso en el banco.</i>", parse_mode="HTML")
 except Exception as e:
...
def callback_listener_buttons(call):
 ...
 show_public_commands(mensaje_clonado) # <--- CORREGIDO AQUÍ
...
def check_bin_standalone(message):
 ...
 # 🔥 INTEGRACIÓN CON BINLIST.IO
 bin_data_api = get_bin_info_binlist(bin_number)

 if bin_data_api:
     brand = bin_data_api["brand"]
     card_type = bin_data_api["type"]
     bank_name = bin_data_api["bank"]
     country_name = bin_data_api["country"]
     flag = bin_data_api["flag"]
 else:
     # Fallback a la lista local si la API falla o no tiene el BIN
...
def check_card(message):
 ...
 bot.edit_message_text(f"🚫 🔼🗻 Error en el checker: {str(e)}", message.chat.id, loading_msg.message_id)
